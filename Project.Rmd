...
measures = tibble(hat.vals, stud.res, cook.dis)
```
...
It appears there are **_16_** influential points within the dataset.

-------

```{r simple.model.hat.vs.studres}
dat = tibble(hat.vals, stud.res, cook.dis)
ggplot(aes(x=hat.vals, y=stud.res, size=cook.dis), data=dat) + 
  ...
  geom_hline(yintercept = 2, color='red', alpha=.7, linetype = "dashed")
# influencePlot(simple.model)
```



```{r}
inf.outliers = influencePlot(simple.model)
inf.outliers
```
