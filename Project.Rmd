...
```{r}
min(cv.lasso.model$cvm)
```

In the previous part, I found the top 5 models with the lowest BIC and then used 10-fold cross validation with a mean-MSE criterion to find the lowest MSE model. I'd like compare the that result with the model I determined though LASSO by examining the MSE values.

```{r}
min(cv.lasso.model$cvm) <= mean.mse
```

The expression above compared the MSE of the LASSO model to the other 5 models but it turns out LASSO produces a regression with a higher MSE compared to all.
