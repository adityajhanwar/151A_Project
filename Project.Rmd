...
for (i in baseball$years){
  if      (i >= neg.cont) { new.years = c(new.years, "neg.cont") }
  else if (i >= neg.sal)  { new.years = c(new.years, "neg.sal")   }
...
sal2 = ggplot(data=new.baseball, aes(y=salary)) + geom_boxplot() + ggtitle("Bocplot of Salary")
...
```{r log.salary.hist}
log.sal1 = ggplot(data=new.baseball, aes(x=log(salary))) + geom_histogram(bins=40) + ggtitle("Histogram of Salary")
...
```{r}
lm(log(salary) ~ (yrneg.cont + yrneg.sal)*log(1+CR), data=new.baseball)
```
